...
class SEMGDataset(Dataset):
    def __init__(self, csv_file, sequence_length):
        ...
        # Assume sEMG PCA channels are in columns 65 to 67 (with column 0 being time).
        self.emg_cols = self.df.columns[65:68].tolist()
...
class sEMGLSTM(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=32, num_layers=2, output_dim=60, dropout=0.5):
...
# ----- Training Setup -----
csv_file = '/kaggle/input/felix070325-3pca-combined/CombinedDatasets3PCA_Felix070325_ABCDE.csv'
sequence_length = 39  
batch_size = 10000
...
model = sEMGLSTM(input_dim=3, hidden_dim=32, num_layers=2, output_dim=60, dropout=0.5).to(device)
...
for epoch in range(num_epochs):
    ...
    if test_mse < best_test_mse:
        ...
        # Save the F2(3) node's predicted and true XYZ values.
...
if best_test_pred_F2_3 is not None and best_test_true_F2_3 is not None:
    # Plot the true versus predicted values for the F2(3) node (XYZ).
    ...
    plot_save_path = 'PredViz_F2(3)_3PCA1.png'
